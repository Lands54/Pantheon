...
class HermesLimiter:
    ...
    def acquire(self, project_id: str, name: str, max_concurrency: int, rate_per_minute: int):
        ...
        with self._lock:
            ...
            if len(q) >= max(1, rate_per_minute):
                raise HermesError(HERMES_RATE_LIMITED, f"Rate limit exceeded for {name}", retryable=True)

            running = self._running.get(key, 0)
            if running >= max(1, max_concurrency):
                raise HermesError(HERMES_BUSY, f"Concurrency limit exceeded for {name}", retryable=True)

            self._running[key] = running + 1
            q.append(now)

    def release(self, project_id: str, name: str):
        key = self._key(project_id, name)
        with self._lock:
            running = self._running.get(key, 0)
            if running <= 1:

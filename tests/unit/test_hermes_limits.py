...
def test_hermes_limits_concurrency_and_rate():
    ...
    lim.acquire(project_id, name, max_concurrency=1, rate_per_minute=10)
    try:
        try:
            lim.acquire(project_id, name, max_concurrency=1, rate_per_minute=10)
        ...
        lim.release(project_id, name)

    name2 = "n2"
    lim.acquire(project_id, name2, max_concurrency=1, rate_per_minute=1)
    lim.release(project_id, name2)
    try:
        lim.acquire(project_id, name2, max_concurrency=1, rate_per_minute=1)
